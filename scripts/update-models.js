...
function generateTypeScript(categories) {
  ...
  for (const [provider, models] of Object.entries(categories)) {
    ...
    if (models.length > 0) {
      ...
      for (const model of models) {
        ...
        lines.push(`    supportsContinuation: ${capabilities.supportsContinuation},`)
  ...
  lines.push('export function findModelData(model: string): ModelData | undefined {')
  
  for (const dataObj of modelDataObjects) {
    lines.push(`  if (${dataObj}[model as keyof typeof ${dataObj}]) {`)
    lines.push(`    return ${dataObj}[model as keyof typeof ${dataObj}] as ModelData`)
    lines.push(`  }`)
  }
